...
def cal_bio_mean(peaks, exposure_period_df):
    ...
    '''
    print(exposure_period_df)

    # Ensure s_time and e_time are numeric
    s_time = float(exposure_period_df["s_time"].iloc[-1])

    # Convert e_time to numeric, replace invalid entries ('Raw') with s_time
    e_time_val = pd.to_numeric(exposure_period_df["e_time"].iloc[-1], errors="coerce")
    if pd.isna(e_time_val):
        e_time_val = s_time
    e_time = float(e_time_val)

    # Select peaks within the exposure period
    bio_df = peaks.loc[
        (peaks["timestamp"] >= s_time) & (peaks["timestamp"] <= e_time)
    ].copy()

    #bio_df = peaks.loc[(peaks["timestamp"] >= float(exposure_period_df["s_time"].iloc[-1])) & (peaks["timestamp"] <= float(exposure_period_df["e_time"].iloc[-1]))].copy()
...
def cal_bio_mean(peaks, exposure_period_df):
    ...
    # Ensure s_time and e_time are numeric
    print(exposure_period_df)
    numeric_df = exposure_period_df.copy()
    numeric_df["s_time"] = pd.to_numeric(numeric_df["s_time"], errors="coerce")
    numeric_df["e_time"] = pd.to_numeric(numeric_df["e_time"], errors="coerce")

    # Drop rows where s_time is invalid
    numeric_df = numeric_df.dropna(subset=["s_time"])

    # Replace invalid e_time with s_time
    numeric_df["e_time"].fillna(numeric_df["s_time"], inplace=True)

    # Take the last valid row
    start_bio = float(numeric_df["s_time"].iloc[-1])
    end_bio = float(numeric_df["e_time"].iloc[-1])

    # Select peaks within the exposure period
    bio_df = peaks.loc[
        (peaks["timestamp"] >= start_bio) & (peaks["timestamp"] <= end_bio)
    ].copy()

    # Prepare lists to store time intervals and mean HR
    s_times, e_times, mean_hr = [], [], []

    i_time = start_bio
    while i_time < end_bio and adjust_time(i_time + 10) <= end_bio:
        i_time_adj = adjust_time(i_time)
        j_time = adjust_time(i_time + 10)

        bio_slice = bio_df.loc[
            (bio_df["timestamp"] >= i_time_adj) & (bio_df["timestamp"] <= j_time)
        ].copy()

        s_times.append(i_time_adj)
        e_times.append(j_time)
        # Use safe mean calculation: skip empty slices
        mean_hr.append(bio_slice["HR"][1:].mean() if not bio_slice.empty else float('nan'))

        i_time += 10

    mean_hr_dic = {"s_time": s_times, "e_time": e_times, "mean_hr": mean_hr}
    bio_mean = pd.DataFrame(mean_hr_dic)
...
def basic_features(peaks, exposure_period_df):
    '''
    Calculates the basic HRV features of the patient safely.
    '''
    valid_peaks = []

    # Convert s_time and e_time to numeric, replace invalid e_time with s_time
    numeric_df = exposure_period_df.copy()
    numeric_df["s_time"] = pd.to_numeric(numeric_df["s_time"], errors="coerce")
    numeric_df["e_time"] = pd.to_numeric(numeric_df["e_time"], errors="coerce")
    numeric_df = numeric_df.dropna(subset=["s_time"])  # Drop invalid s_time
    numeric_df["e_time"].fillna(numeric_df["s_time"], inplace=True)

    for j in range(1, min(18, len(numeric_df))):
        FD = []
        NNI_FD = []

        s = numeric_df["s_time"].iloc[j]
        e = numeric_df["e_time"].iloc[j]

        valid = peaks.loc[(peaks["timestamp"] >= s) & (peaks["timestamp"] <= e)].copy()
        if valid.empty:
            # If no peaks in this interval, skip
            valid_peaks.append(valid)
            continue

        for i in range(0, len(valid.index) - 1):
            f_diff = abs(valid["HR"].iloc[i + 1] - valid["HR"].iloc[i])
            f_diff_nn = abs(valid["NNI"].iloc[i + 1] - valid["NNI"].iloc[i])
            FD.append(f_diff)
            NNI_FD.append(f_diff_nn)
        ...
        valid.insert(0, "event", [numeric_df["event"].iloc[j]] * len(valid))
        valid["NNI_FD"] = NNI_FD
        valid["FD"] = FD

        # Second difference (SD) safely
        SD = []
        for i in range(0, len(valid.index) - 2):
            s_diff = abs(valid["HR"].iloc[i + 2] - valid["HR"].iloc[i])
            SD.append(s_diff)

        # Pad SD to match valid length
        n_pad = len(valid) - len(SD)
        SD = [0]*n_pad + SD
        valid["SD"] = SD
...
def adv_features(peaks, exposure_period_df):
    '''
    Calculates the advanced HRV features of the patient safely.
    '''
    # Convert s_time and e_time to numeric
    numeric_df = exposure_period_df.copy()
    numeric_df["s_time"] = pd.to_numeric(numeric_df["s_time"], errors="coerce")
    numeric_df["e_time"] = pd.to_numeric(numeric_df["e_time"], errors="coerce")
    numeric_df = numeric_df.dropna(subset=["s_time"])
    ...
    mean_biofeedback = cal_bio_mean(peaks, numeric_df)
    valid_peaks = basic_features(peaks, numeric_df)

    for i in range(min(17, len(valid_peaks))):
        window = valid_peaks[i]
        if window.empty:
            continue

        start_bio = numeric_df["s_time"].iloc[i + 1]
        end_bio = numeric_df["e_time"].iloc[i + 1]

        s_time, e_time, mean_hr, std_hr, NFD, NSD = [], [], [], [], [], []
        avNN, sdNN, HRV, RMSSD, NN50, pNN50, pNN20, event, window_list = [], [], [], [], [], [], [], [], []

        i_time = adjust_time(start_bio)
        k = 1

        while i_time < end_bio and adjust_time(i_time + 10) <= end_bio:
            window_slice = window.loc[
                (window["timestamp"] >= i_time) & (window["timestamp"] <= adjust_time(i_time + 10))
            ].copy()
            ...
            event.append(window_slice["event"].iloc[0])
            window_list.append(k)
            s_time.append(i_time)
            e_time.append(adjust_time(i_time + 10))
            mean_hr.append(window_slice["HR"].mean())
            std_hr.append(window_slice["HR"].std(ddof=1))
            NFD.append(window_slice["FD"][1:].mean())
            NSD.append(window_slice["SD"][2:].mean())
        ...
        mean_hr_dic = {
            "event": event, "window": window_list, "s_time": s_time, "e_time": e_time,
            "mean_hr": mean_hr, "std": std_hr, "NFD": NFD, "NSD": NSD,
            "HRV": HRV, "avNN": avNN, "sdNN": sdNN, "RMSSD": RMSSD,
            "NN50": NN50, "pNN50": pNN50, "pNN20": pNN20
        }
        video_mean_df.append(pd.DataFrame(mean_hr_dic))

    if video_mean_df:
        final_df = pd.concat(video_mean_df, ignore_index=True)
    else:
        final_df = pd.DataFrame()  # Return empty DF if no data
